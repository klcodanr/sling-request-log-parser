    ...
    <table class="table table-sm">
      <thead>
        <tr>
          ...
          <th class="table-col-sm" title="The total time the operation took, including child operations">
            Duration (μs)
          </th>
          <th class="table-col-sm" title="The total time only this operation took, subtracting child operations">
      ...
      <tbody>
        <tr v-for="line in lines" :key="line.time" :class="{
          ...
          <td>{{ line.type }}</td>
          <td>
            <span v-if="line.duration">{{ (line.duration / 1000000.0).toFixed(4) }} s <small>({{
              line.duration.toLocaleString() }} μs)</small></span>
          </td>
          <td><span v-if="line.executionDuration">{{ (line.executionDuration / 1000000.0).toFixed(4) }} s <small>({{
            line.executionDuration.toLocaleString() }} μs)</small><br /><em>{{
    (
