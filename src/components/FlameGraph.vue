...
let chart;
onMounted(() => {
  const details = document.getElementById('flame-graph--details');
  chart = flamegraph()
    .width(1050)
    .cellHeight(24)
    .setColorMapper((el, originalColor) => {
      ...
      if (
        el.data.name.includes('/libs') ||
        el.data.name.includes('com.day') ||
        el.data.name.includes('com.adobe')
      ) {
        return '#f68d2e';
      }
      if (
        el.data.name.includes('org.apache') ||
        ['handleSecurity', 'ResourceResolution', 'ServletResolution'].includes(
          el.data.name,
        )
      ) {
        return '#D22128';
      ...
      if (
        el.data.name.includes('/apps') ||
        el.data.name.includes('/mnt') ||
        el.data.name.includes('/conf') ||
        el.data.name.includes('/content') ||
        el.data.name.includes('com.')
      ) {
        return '#ffc107';
      }
      return '#adb5bd';
